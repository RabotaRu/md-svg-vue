<script>
  export default {
    name: 'md-signal-wifi-3-bar',

    serverCacheKey: props => {
      return `md-signal-wifi-3-bar:${props.color || 'default'}:${props.width}:${props.height}:${props.className || 'icon'}`;
    },
    ...
    render (h) {
      const data = {
        staticClass: 'icon md-icon md-signal-wifi-3-bar',
